...
class DoubleBiLSTM_CRF_Model(ABCLabelingModel):
    @classmethod
    def default_hyper_parameters(cls) -> Dict[str, Dict[str, Any]]:
        return {
            'layer_blstm1': {
    ...
    def build_model_arc(self) -> None:
        ...
        layer_stack = [
            L.Bidirectional(L.LSTM(**config['layer_blstm1']), name='layer_blstm1'),
            L.Bidirectional(L.LSTM(**config['layer_blstm2']), name='layer_blstm2'),
            L.Dropout(**config['layer_dropout'], name='layer_dropout'),
            L.TimeDistributed(L.Dense(output_dim, **config['layer_time_distributed'])),
            crf
        ]
    ...
    def compile_model(self,
        ...
        if metrics is None:
            metrics = [self.crf_layer.accuracy]
        super(DoubleBiLSTM_CRF_Model, self).compile_model(loss=loss,
                                                          optimizer=optimizer,
                                                          metrics=metrics,
                                                          **kwargs)
